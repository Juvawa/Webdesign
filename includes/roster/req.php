...
class rosterRequest
{
	function reqAreas()
	{
		...
		$areas = $database -> doSet('EMPLOYEE','AREAS');
		return $areas;
	}
	...
	{
		$database = new Database();
      if($result = $database->doRows("SELECT USER_ID,NAME,SURNAME,DOB,PHONE,EMAIL,USERNAME,HOURS,AREAS,ACTIVE,CONFIRMED FROM EMPLOYEE WHERE USER_ID='$ID' LIMIT 1"))
		{
			return $result['0'];
		}
	}
